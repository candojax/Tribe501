	...
	*/
	public function display_cart() {
		...
		if ($isCheckout !== true) {
			...
			echo tab(7) . "<input type='$inputType' $src id='jcart-checkout' name='jcartCheckout' class='jcart-button btn' value='{$config['text']['checkout']}' />\n";
		}
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				echo tab(7) . "<input id='jcartItemQty-{$item['id']}' name='jcartItemQty[]' class='input-mini' type='text' value='{$item['qty']}' />\n";
				echo tab(6) . "</td>\n";
				echo tab(6) . "<div class='span3'><td class='jcart-item-name'>\n";

				if ($item['url']) {
					echo tab(7) . "<a href='{$item['url']}'>{$item['name']}</a></div>\n";
				}
				else {
		...
		if ($isCheckout === true) {
			...
			echo tab(3) . "<input class='btn' type='$inputType' $src id='jcart-paypal-checkout' name='jcartPaypalCheckout' value='{$config['text']['checkoutPaypal']}' $disablePaypalCheckout />\n";
		}
